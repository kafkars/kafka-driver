//! Bounded loopback broker framing for the coordinator-routing scenario.
...
use kafka_driver::{ApiVersion, BootstrapLimits, BootstrapSet, BrokerEndpoint, HostName};
use kafka_wire::{
    ApiVersionsRequest, ApiVersionsResponse, FindCoordinatorRequest, FindCoordinatorResponse,
    KafkaMessage, MetadataRequest, MetadataResponse, RequestHeader as WireRequestHeader,
    ResponseHeader, metadata_response::MetadataResponseBroker, request_header_version,
...
pub(super) fn accept_after_driving(
    ...
    listener
        ...
        .unwrap_or_else(|error| panic!("make coordinator listener nonblocking: {error}"));
    for _ in 0..16 {
        drive(
            reactor,
            Duration::from_millis(100),
            "open coordinator child",
        );
        match listener.accept() {
            Ok((peer, _)) => {
                peer.set_nonblocking(false)
                    .unwrap_or_else(|error| panic!("make coordinator peer blocking: {error}"));
                return peer;
            }
            Err(error) if error.kind() == std::io::ErrorKind::WouldBlock => {}
            Err(error) => panic!("accept coordinator: {error}"),
        }
    }
    panic!("coordinator child did not connect: {reactor:?}");
}

pub(super) fn wait_for_frame(peer: &TcpStream, reactor: &mut kafka_driver::Reactor) {
    peer.set_nonblocking(true)
        .unwrap_or_else(|error| panic!("make coordinator peer nonblocking: {error}"));
    let mut byte = [0; 1];
    for _ in 0..16 {
        drive(
            reactor,
        ...
        match peer.peek(&mut byte) {
            ...
            Ok(_) => {}
            Err(error) if error.kind() == std::io::ErrorKind::WouldBlock => {}
            Err(error) => panic!("inspect coordinator request: {error}"),
        }
    }
    panic!("coordinator call was not written: {reactor:?}");
}

pub(super) fn read_find_coordinator_request(peer: &mut TcpStream) -> FindCoordinatorRequestFrame {
    let frame = read_frame(peer);
    let version = ApiVersion::new(read_i16(&frame, 2));
    let header_version = request_header_version(FindCoordinatorRequest::is_flexible(version));
    let bytes = Bytes::from(frame);
    let mut decoder = Decoder::new(bytes.clone(), DecodeLimits::default())
        .unwrap_or_else(|error| panic!("construct coordinator request decoder: {error}"));
    let header = WireRequestHeader::decode(&mut decoder, ApiVersion::new(header_version))
        .unwrap_or_else(|error| panic!("decode coordinator request header: {error}"));
    let request = FindCoordinatorRequest::decode_from_bytes(
        bytes.slice(decoder.offset()..),
        version,
        DecodeLimits::default(),
    )
    .unwrap_or_else(|error| panic!("decode FindCoordinator request: {error}"));
    FindCoordinatorRequestFrame {
...
pub(super) fn metadata_response(
    ...
    response.brokers = vec![broker(1, seed_port), broker(7, coordinator_port)];
    response.controller_id = 1;
    encoded_response::<MetadataRequest, _>(correlation_id, &response, ApiVersion::new(1))
}

pub(super) fn find_coordinator_response(correlation_id: i32, coordinator_port: u16) -> Vec<u8> {
    let mut response = FindCoordinatorResponse::default();
    response.node_id = 7;
    response.host = StrBytes::from("127.0.0.1");
    response.port = i32::from(coordinator_port);
    encoded_response::<FindCoordinatorRequest, _>(correlation_id, &response, ApiVersion::new(3))
}
...
pub(super) struct FindCoordinatorRequestFrame {
    pub(super) correlation_id: i32,
    pub(super) request: FindCoordinatorRequest,
}
